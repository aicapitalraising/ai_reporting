-- Fix: showed_calls should be counted by scheduled_at (actual appointment date),
-- not by booked_at (when the appointment was created).
-- Also: add crm_leads count that includes ALL GHL leads regardless of email/phone,
-- so the dashboard can detect GHL sync issues when ad spend exists but no CRM leads.
...
AS $function$
  WITH lead_stats AS (
    SELECT
      l.client_id,
      -- Quality leads: non-spam with both email and phone
      COUNT(*) FILTER (
      ...
      -- ALL CRM leads (everything synced from GHL, regardless of email/phone)
      COUNT(*) AS crm_leads
    FROM public.leads l
  ...
  -- Booked calls: counted by booked_at (when appointment was created)
  ...
  -- Showed calls: counted by scheduled_at (actual appointment date when they showed)
