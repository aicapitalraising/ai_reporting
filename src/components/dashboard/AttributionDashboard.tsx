...
import { TrendingUp, Layers, Target, Calendar } from 'lucide-react';
...
export function AttributionDashboard({ leads, calls, fundedInvestors }: AttributionDashboardProps) {
  ...
  }, [filteredLeads, filteredCalls, filteredFundedInvestors, leadMap, view]);
      ...
      <CardContent className="space-y-6">
        {!hasData ? (
          <div className="text-center py-8 text-muted-foreground">
            <p>No attribution data available for the selected date range.</p>
            <p className="text-sm mt-1">Try adjusting the date range or check if leads have campaign data.</p>
          </div>
        ) : (
          <>
            {/* Bar Chart */}
