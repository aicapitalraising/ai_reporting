...
export function KPIGrid({ metrics, onMetricClick, showFundedMetrics = false }: KPIGridProps) {
  const kpis = [
    ...
    { key: 'costPerLead', label: 'Cost Per Lead', value: metrics.costPerLead ?? 0, format: 'currency' as const, change: 0 },
    { key: 'calls', label: 'Calls', value: metrics.calls ?? metrics.totalCalls ?? 0, format: 'number' as const, change: 0, clickable: true },
    { key: 'costPerCall', label: 'Cost Per Call', value: metrics.costPerCall ?? 0, format: 'currency' as const, change: 0 },
    ...
    { key: 'costPerShow', label: 'Cost Per Show', value: metrics.costPerShow ?? 0, format: 'currency' as const, change: 0 },
    ...
    { key: 'fundedInvestors', label: 'Funded Investors', value: metrics.fundedInvestors ?? 0, format: 'number' as const, change: 0, clickable: true },
    { key: 'fundedDollars', label: 'Funded $', value: metrics.fundedDollars ?? 0, format: 'currency' as const, change: 0 },
    { key: 'costPerInvestor', label: 'Cost / Investor', value: metrics.costPerInvestor ?? 0, format: 'currency' as const, change: 0 },
    { key: 'costOfCapital', label: 'Cost of Capital', value: metrics.costOfCapital ?? 0, format: 'percent' as const, change: 0 },
  ];
